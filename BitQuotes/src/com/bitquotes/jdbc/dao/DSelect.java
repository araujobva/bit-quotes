...
public class DSelect {
    ...
    public static ArrayList authentication() {
        try {
            ...
            //SELECT us_name, us_password, us_administrator FROM user
            String query = "SELECT us_name, us_password, us_administrator "
                    + "FROM user";
    ...
    public static ArrayList searchBookName(String user, String bookName) {
        try {
            Connection con = JConnectionFactory.getConnection();
            //SELECT quote.qu_id, quote.qu_quote, book.bo_name, quote.qu_book_page FROM quote INNER JOIN book ON book.bo_id = quote.bo_id WHERE quote.us_name = "NomeUsuário" AND book.bo_name = "NomeLivro";
